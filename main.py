...
async def command_start_handler(message: Message) -> None:
    ...
    if user:
        if message.from_user.id == 535185511:
            await bot.send_message(chat_id=message.chat.id, text=text, reply_markup=register_admin_keyboard())
        else:
        ...
        await bot.send_message(chat_id=message.chat.id, text=text_register, reply_markup=register_user_keyboard())
...
async def back_start_handler(callback_query: CallbackQuery, state: FSMContext) -> None:
    ...
    if callback_query.from_user.id == 535185511:
        await bot.send_message(chat_id=callback_query.from_user.id, text=text, reply_markup=register_admin_keyboard())
    else:
